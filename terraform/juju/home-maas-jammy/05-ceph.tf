...
resource "juju_application" "ceph-radosgw" {
  ...
  config = {
      ...
      os-admin-hostname    = "${join(".",[var.fqdn-admin["radosgw"],var.domain])}"
      os-internal-hostname = "${join(".",[var.fqdn-int["radosgw"],var.domain])}"
      os-public-hostname   = "${join(".",[var.fqdn-pub["radosgw"],var.domain])}"
  }
}
